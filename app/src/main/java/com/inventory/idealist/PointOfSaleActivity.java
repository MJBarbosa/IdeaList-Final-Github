...
// Set a click listener for the "Decrease Quantity" button
    ...
    private void addToCart() {
        ...
        // Check if the cartItem matches the default text
        if (!cartItem.equals("Product Name: ,Product Description: ,Category: ,Quantity: 0,Price: ,This is an invisible TextView")) {
            ...
            // Display an error message if the cartItem matches the default text
            Toast.makeText(this, "Invalid product data. Please enter valid details.", Toast.LENGTH_SHORT).show();
        }
    }
        ...
        if (intent != null && intent.hasExtra("userUid") && intent.hasExtra("scannedContent")) {
            ...
            if (userUid != null && !userUid.isEmpty() && scannedContent != null && !scannedContent.isEmpty()) {
                ...
                if (product != null) {
                    ...
                    onQRCodeScanned(product.getProductId(), product.getProductName());
    ...
    private void showSearchDialog() {
        ...
        buttonPopulateDetails.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String selectedItem = autoCompleteTextViewSearch.getText().toString();
                PointOfSaleActivity.Product selectedProduct = findProduct(selectedItem);
                if (selectedProduct != null) {
                    populateUIWithProductData(selectedProduct);
                } else {
    ...
    private void populateUIWithProductData(PointOfSaleActivity.Product selectedProduct) {
        // Use the selected product to populate the UI
        String labelProductName = "Product Name: ";
        String labelProductDescription = "Product Description: ";
        String labelCategory = "Category: ";
        String labelPrice = "Price: ";
    ...
    private void populateUIWithProductDataById(String productId) {
        // Find the corresponding product in the productList
        PointOfSaleActivity.Product selectedProductById = findProductById(productId);
        ...
        if (selectedProductById != null) {
            ...
            String labelPrice = "Price: ";
            // Populate the editText fields with the selected product's data
            textViewPOSProductDescription.setText(labelProductDescription + selectedProductById.getProductDescription());
            textViewPOSPrice.setText(labelPrice + selectedProductById.getPrice());
            textViewPOSCategory.setText(labelCategory + selectedProductById.getCategory());

            // Update the TextViews for productId and productName
            TextView textViewPOSProductName = findViewById(R.id.textViewPOSProductName);

            textViewPOSProductName.setText(labelProductName + selectedProductById.getProductName());

            // Display the product image
            ImageView imageViewProductImage = findViewById(R.id.imageViewPOSProductImage);
            displayProductImage(selectedProductById.getUserUid(), selectedProductById.getProductId(), imageViewProductImage);
        } else {
            Log.e(TAG, "Selected product is null.");
        }
    }
    ...
    private void setupSearch() {
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                // Create an array of product names and IDs for the search suggestions
                List<String> productNamesAndIds = new ArrayList<>();
                for (PointOfSaleActivity.Product product : productList) {
                    productNamesAndIds.add(product.getProductName());
                    productNamesAndIds.add(product.getProductId());
                }

                // Add product names and IDs to the searchAdapter
                searchAdapter.addAll(productNamesAndIds);
        ...
        autoCompleteTextViewPOSSearch.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (selectedProduct != null) {
                    if (selectedItem.equals(selectedProduct.getProductName())) {
                        // Handle the case where the selected item is a product name
                        populateUIWithProductData(selectedProduct);
                    } else if (selectedItem.equals(selectedProduct.getProductId())) {
                        // Handle the case where the selected item is a product ID
                        populateUIWithProductDataById(selectedProduct.getProductId());
                    } else {
                        Log.e(TAG, "Selected product is null.");
                    }
                } else {
    ...
    private PointOfSaleActivity.Product findProduct(String item) {
        // Find the product in the productList based on the item (could be a name or an ID)
        for (PointOfSaleActivity.Product product : productList) {
            if (item.equals(product.getProductName()) || item.equals(product.getProductId())) {
                return product;
            }
        }
    ...
    private PointOfSaleActivity.Product findProductById(String productId) {
        for (PointOfSaleActivity.Product product : productList) {
